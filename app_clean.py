# app_clean.py — IC-LicAI Expert Console (Locked Build, Structural + FRAND + Narrative Upgrade v2)
...
# IAS 38 Structural Capital emphasis (dominance on explicit artefacts),
# FRAND-aware licensing templates, Seven Stakeholder / ESG narrative,
# and "Company" terminology in the UI.
...
FOUR_LEAF_KEYS: Dict[str, List[str]] = {
    ...
    "Customer": [
        ...
        "quote",
        "proposal",
        # explicit deal docs are handled as Structural; customer here stays largely tacit/relational
        "purchase order",
        "po",
        "invoice",
        "renewal",
...
def _analyse_weighted(
    ...
    for fname, w in (weights_by_file or {}).items():
        ...
        # Knowledge / SOP / KMP / safety / ISO / CRM → Structural + Human
        if any(k in n for k in ["knowledge", "kmp", "sop", "process", "safety", "protocol", "risk", "qms", "iso", "crm"]):
    ...
    # If explicit structural cues are present, Structural should dominate
    # the tacit leaves (Human / Customer / Strategic Alliance) as the IAS 38-ready category.
    if explicit_any:
        max_other = max(
            leaf_scores[k] for k in leaf_scores.keys() if k != "Structural"
        ) if any(k != "Structural" for k in leaf_scores.keys()) else 0.0
        if max_other > 0:
            # Ensure Structural is at least 10% higher than the strongest tacit leaf
            leaf_scores["Structural"] = max(leaf_scores["Structural"], max_other * 1.1)
    ...
    for leaf, score in leaf_scores.items():
        ...
        if leaf == "Human":
            ...
            nar = (
                ...
                if tick
                else "Structural Capital is under-documented; explicit artefacts (contracts, registers, SOPs, CRM, board packs, "
                "pricing, datasets) should be consolidated into an auditable IA Register."
            )
        elif leaf == "Customer":
            nar = (
                "Customer Capital is evidenced through renewals, channels and relationship patterns, supporting recurring value "
                "capture and future licensing opportunities once linked to explicit Structural assets."
                if tick
                else "Customer Capital appears weak in the evidence; contracts, renewals, CRM and pipeline data should be structured "
                "and connected to the IA Register."
            )
        else:
            nar = (
...
def _build_interpreted_summary(
    ...
    p2b = (
        "Evidence points to maturing Structural Capital where explicit artefacts — contracts, SOPs, protocols, registers, CRM, "
        "board materials and datasets — are present. These are the primary candidates for IAS 38-compliant recognition on "
    ...
    p4_mid = (
        ...
        "protocols, CRM and registers) can support multiple simultaneous value streams — including revenue licences, access "
        "or community licences, co-creation arrangements and data/algorithm sharing — provided that ownership, rights and "
        "governance are clarified."
    )

    actions = [
        "Create a single IA Register linking all explicit artefacts (contracts, JVs, SOPs, protocols, CRM, datasets, board packs) to the 4-Leaf Model.",
...
# 4) REPORTS
    ...
    def _compose_ic() -> Tuple[str, str]:
        ...
        b.append(
            ...
            if PUBLIC_MODE
            else "CONFIDENTIAL. Advisory-first; expert review required for final scoring, licensing design and accounting treatment."
        )
        return title, "\n".join(b)
    ...
    def _compose_lic() -> Tuple[str, str]:
        ...
        b.append(
            "- Revenue licences: royalty or fee-based licences (e.g. per unit, per user, per dataset) with "
        ...
        b.append(
            ...
            "processes, brand, training content, CRM) to its licensing model(s)."
        )
        b.append(
