# app_clean.py — IC-LicAI Expert Console (Locked Build + Bulk Context Paste)
# Adds: DOCX/PPTX extraction, weighted IC signal engine, interpreted narrative,
# evidence dashboard (quality, file-type mix, radar), expert-context fusion,
# defensive handling for stale session_state["ten_steps"],
# and ONE accessibility feature: bulk expert-context paste → auto-fill fields.
...
import plotly.graph_objects as go  # for radar / simple bar
...
<style>
  ...
  .ic-title-bar{
    background:#0F2F56; color:#fff; font-weight:800; font-size:34px;
...
def _safe(name: str) -> str:
    return (
        "".join(c for c in (name or "").strip() if c.isalnum() or c in (" ", "_", "-", "."))
        .strip()
        .replace(" ", "_")
    )
    ...
    if HAVE_DOCX:
        ...
        if not PUBLIC_MODE:
            doc.add_paragraph().add_run(
                "CONFIDENTIAL — Internal Evaluation Draft (No Distribution)"
            ).bold = True
...
def _analyse_weighted(
    text: str, weights_by_file: Dict[str, float]
    ...
    for leaf, score in leaf_scores.items():
        ...
        if leaf == "Human":
            nar = (
                ...
                if not tick
                else "Human Capital evidenced (values/awards/training/safety) but requires competency mapping."
            )
        elif leaf == "Structural":
            nar = (
                "Structural Capital under-documented; SOPs/KMP/ISO mapping recommended."
                if not tick
                else "Structural Capital formalised (KMP/SOPs/process maps/ISO/QMS) enabling repeatable delivery."
            )
        elif leaf == "Customer":
            nar = (
                "Customer Capital weak in evidence; contracts/renewals/pipeline should be documented."
                if not tick
                else "Customer Capital present (contracts/POs/channels), supporting recurring value capture."
            )
        else:
            nar = (
                "Strategic alliances not clearly evidenced; JV/MoU documentation needed."
                if not tick
                else "Strategic Alliance Capital evidenced (JV/MoU/partners/universities/councils)."
            )
    ...
    weight_mean = (
        (sum(weights_by_file.values()) / max(1, len(weights_by_file)))
        if weights_by_file
        else 0.4
    )
    quality = int(
...
def _build_interpreted_summary(
    ...
    p1 = (
        f"{case} is a {size} in {sector}. Based on uploaded artefacts and expert context, the company shows "
    ...
    if strengths:
        p2a = (
            f"Strengths concentrate in {', '.join(strengths)}"
            + (f"; gaps are {', '.join(gaps)}." if gaps else ".")
        )
    else:
    ...
    p2b = (
        ...
        "once competency maps and CRM/renewal data are attached."
    )
    p2 = p2a + " " + p2b
    ...
    actions = [
        "Create a single IA Register linking contracts/JVs, SOPs/KMP, product/service artefacts and ESG evidence.",
        "Introduce quarterly governance reporting (board pack + KPI dashboard) to strengthen Monitor and Report.",
        "Define valuation approach (IAS 38 fair value) and connect it to licensing options to create multiple simultaneous value streams.",
        "Formalise competency matrices and training logs to evidence Human Capital maturity and reduce key-person risk.",
    ]
    p4 = "Assumptions & Action Plan:\n" + "\n".join([f"• {a}" for a in actions])

    p5 = (
        f"Evidence quality ≈ {evidence_quality}% (heuristic). "
        "Using the 7 Stakeholder Model as defined by Professor Philip Sugai "
        "(employees, customers, investors, partners, suppliers, communities and the natural environment), "
        "future iterations should strengthen double materiality links between ESG scores and intangible asset value. "
        "Request additional artefacts: CRM/renewal data, NDA/licence/royalty terms, ESG/impact metrics, "
        "and board/management reports."
    )
...
# --------------- BULK CONTEXT PARSER -----------------
def _parse_bulk_context(txt: str) -> Dict[str, str]:
    """
    Very simple helper: take one pasted block and try to map to the 6 expert answers.
    Strategy:
      1) Split into non-empty lines.
      2) Strip optional 'Label:' prefixes (Why service:, Stage:, etc.).
      3) Map line 0..5 -> fields in fixed order.
    """
    out = {
        "why": "",
        "stage": "",
    ...
    LABELS = {
        "why": ["why service", "why is the customer seeking this service"],
        "stage": ["stage"],
        "plan_s": ["short-term", "short term"],
        "plan_m": ["medium-term", "medium term"],
        "plan_l": ["long-term", "long term"],
        "markets": ["markets", "fit and why"],
        "sale": ["sale", "price", "valuation"],
    }
        ...
        for key, labs in LABELS.items():
            for lab in labs:
                if lower.startswith(lab + ":"):
                    return line[len(lab) + 1 :].strip()
        return line

    lines = [_strip_label(l) for l in lines]

    order_keys = ["why", "stage", "plan_s", "plan_m", "plan_l", "markets", "sale"]
    for i, line in enumerate(lines[: len(order_keys)]):
        out[order_keys[i]] = line
...
ss.setdefault("evidence_counts", {})
ss.setdefault("evidence_weights", {})
# Expert prompts
...
ss.setdefault("bulk_context", "")  # NEW: one big paste box
...
# 1) CUSTOMER (with required prompts + bulk paste)
if page == "Customer":
    ...
    with st.form("customer_form"):
        ...
        with c1:
            case_name = st.text_input(
                "Customer / Company name *", ss.get("case_name", "")
            )
        with c2:
            size = st.selectbox(
                "Company size",
        ...
        # NEW: bulk paste box
        bulk_context = st.text_area(
            "Optional: paste full expert context here (one block, then auto-fill below)",
            ss.get("bulk_context", ""),
            height=120,
            help=(
                "Accessibility helper: paste all answers once (e.g. from Notes). "
                "On Save, the app will split this into the fields below in order."
            ),
        )
        auto_split = st.checkbox(
            "Auto-split pasted context into fields on Save",
            value=True,
            help="If checked, the big pasted block above is used to fill all fields below.",
        )

        # Individual fields (will be auto-filled from bulk on submit)
        ...
        stage = st.text_area(
            "2) What stage are the products/services at? *",
            ss.get("stage", ""),
            height=90,
        )
        ...
        with c4:
            plan_s = st.text_area(
                "3a) Short-term plan (0–6m) *", ss.get("plan_s", ""), height=90
            )
        with c5:
            plan_m = st.text_area(
                "3b) Medium-term plan (6–24m) *", ss.get("plan_m", ""), height=90
            )
        with c6:
            plan_l = st.text_area(
                "3c) Long-term plan (24m+) *", ss.get("plan_l", ""), height=90
            )
        markets_why = st.text_area(
            "4) Which markets fit and why? *", ss.get("markets_why", ""), height=90
        )
        ...
        if submitted:
            # If user provided a bulk block and wants auto-split, use it
            if bulk_context.strip() and auto_split:
                parsed = _parse_bulk_context(bulk_context)
                # Only overwrite if field is currently empty to avoid surprises
                why_service = why_service or parsed["why"]
                stage = stage or parsed["stage"]
                plan_s = plan_s or parsed["plan_s"]
                plan_m = plan_m or parsed["plan_m"]
                plan_l = plan_l or parsed["plan_l"]
                markets_why = markets_why or parsed["markets"]
                sale_price_why = sale_price_why or parsed["sale"]
            ...
            if missing_fields:
                ...
                ss["bulk_context"] = bulk_context.strip()
                if uploads:
                    ss["uploads"] = uploads
    ...
    if ss.get("uploads"):
        st.info(
            f"{len(ss['uploads'])} file(s) stored in session. Go to **Analyse Evidence** next."
        )

# 2) ANALYSE EVIDENCE  (Evidence dashboard + run button)
elif page == "Analyse Evidence":
    st.header("Evidence Dashboard & Analysis")

    # Top row: Evidence quality + file-type mix
    ...
    with col1:
        ...
        st.caption(
            f"{eq}% evidence coverage (heuristic – based on file mix, diversity, and weight)."
        )

    with col2:
        st.subheader("File-type mix")
        counts: Dict[str, int] = ss.get("evidence_counts", {}) or {}
        if not counts:
            st.caption(
                "No files analysed yet. Upload evidence on the Customer page and click **Run analysis now** below."
            )
        else:
            labels = list(counts.keys())
            values = [counts[k] for k in labels]
            summary = ", ".join(f"{ext}: {cnt}" for ext, cnt in zip(labels, values))
            st.caption(f"Files analysed by type: {summary}")
    ...
    leaf_scores: Dict[str, float] = ss.get("leaf_scores", {}) or {}

    labels = ["Human", "Structural", "Customer", "Strategic Alliance"]
    raw_vals = [leaf_scores.get(l, 0.0) for l in labels]
    max_val = max(raw_vals) if any(raw_vals) else 1.0
    radar_vals = [0.0 if max_val == 0 else (v / max_val) * 10 for v in raw_vals]

    fig = go.Figure(
        data=go.Scatterpolar(
            r=radar_vals + radar_vals[:1],
            theta=labels + labels[:1],
            fill="toself",
            name="IC strength (relative)",
        )
    )
    fig.update_layout(
    ...
    st.caption(
        "Radar shows relative strength of evidence for each IC leaf (scaled 0–10 from weighted signals)."
    )

    st.markdown("### What we've found / What may be missing")

    colA, colB = st.columns(2)
    with colA:
        st.markdown("**Found (✓)**")
        found = [k for k, v in ic_map.items() if v.get("tick")]
        if found:
            for leaf in found:
                st.markdown(f"- ✓ **{leaf}** — {ic_map[leaf].get('narrative','')}")
        else:
            st.caption("No strong IC signals evidenced yet – analysis will update this once run.")

    with colB:
        st.markdown("**Potential gaps (⚠️)**")
        gaps = [k for k, v in ic_map.items() if not v.get("tick")]
        if gaps:
            for leaf in gaps:
                st.markdown(f"- ⚠️ **{leaf}** — {ic_map.get(leaf,{}).get('narrative','')}")
        else:
            st.caption("No major gaps flagged based on current artefacts.")
    ...
    if st.button("Run analysis now"):
        ...
        ic_map, ten, leaf_scores, quality = _analyse_weighted(
            combined_text_for_detection, weights
        )

        case = ss.get("case_name", "Untitled Customer")
        interpreted = _build_interpreted_summary(
            case, leaf_scores, ic_map, ten, quality, context
        )
        ...
        if len(extracted.strip()) < 100:
            st.warning(
                "Little machine-readable text was extracted (DOCX/PPTX extraction is enabled). "
...
# 3) EXPERT VIEW
    ...
    with colA:
        ...
        st.markdown(
            f"- **Plans:** S={ss.get('plan_s','') or '—'} | M={ss.get('plan_m','') or '—'} | L={ss.get('plan_l','') or '—'}"
        )
...
# 4) REPORTS
    ...
    def _compose_ic() -> Tuple[str, str]:
        ...
        for leaf in ["Human", "Structural", "Customer", "Strategic Alliance"]:
            ...
            b.append(
                f"- {leaf}: {'✓' if row.get('tick') else '•'} — {row.get('narrative','')}{tail}"
            )
    ...
    def _compose_lic() -> Tuple[str, str]:
        ...
        b.append("Models:")
        b.append(
            "- Revenue licence (royalties, FRAND-aligned terms, annual audit clause, multi-stakeholder alignment)"
        )
        b.append("- Defensive licence (IP pooling, non-assert across partners)")
        b.append(
            "- Co-creation licence (shared ownership of Foreground IP, revenue-sharing, ESG-aligned innovation)"
        )
        b.append("\nGovernance & Audit")
        b.append(
            "IA Register maintained; evidence bundles per licence; regular royalty/performance and ESG reporting."
        )
        if PUBLIC_MODE:
    ...
    with c1:
        if st.button("Generate IC Report (DOCX/TXT)", key="btn_ic"):
            ...
            st.download_button(
                "⬇️ Download IC Report", data, file_name=fname, mime=mime, key="dl_ic"
            )
    ...
    if st.button("Generate template", key="btn_make_template"):
        if template == "FRAND Standard":
            ...
            body = (
                ...
                "commercial model, revenue sharing, ESG and impact alignment, exit/assignment, "
                "FRAND alignment where applicable.\n"
            )
        else:
        ...
        st.download_button(
            "⬇️ Download Template", data, file_name=fname, mime=mime, key="dl_tpl"
        )
        (st.success if path else st.warning)(msg)
