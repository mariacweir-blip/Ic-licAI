...
<style>
  ...
  /* Sidebar expander (Glossary) – match sidebar colour, light text on header */
  section[data-testid="stSidebar"] [data-testid="stExpander"] {{
    background: {SIDEBAR_BG} !important;
  }}

  /* Glossary header text (the clickable bar) */
  section[data-testid="stSidebar"] [data-testid="stExpander"] div[role="button"] p {{
    color: #E7F0FF !important;
  }}

  /* Glossary body text – keep dark like other content */
...
# ---- VM assumptions sidebar workspace helper -----------------------------
def vm_assumptions_block(
    ...
    Stores in session_state:
        "vm_assumptions_accepted"  -> List[VMAssumption]
        "vm_assumptions_confirmed" -> bool
        "vm_manual_assumptions"    -> List[VMAssumption]
    """
    with st.sidebar:
...
# 4) LIP CONSOLE (was Expert View)
    ...
    with st.sidebar:
        ...
        for a in suggested:
            ...
            # Always show the assumption text under the checkbox
            st.caption(a.narrative)
            st.caption(
                f"Rationale: {a.rationale} "
                f"(Signals: {', '.join(a.source_signals)} | Confidence: {a.confidence})"
            )
