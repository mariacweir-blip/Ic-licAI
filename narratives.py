...
"""
    ...
    def build_narrative(case: str,
                    ic_map,
                    readiness,
                    licensing):
    """Expanded advisory narrative for IC-LicAI demo."""
    
    assets_total = sum(len(v) for v in ic_map.values())
    strengths = [r.get("name","") for r in readiness if r.get("score", 0) >= 2]
    gaps = [r.get("name","") for r in readiness if r.get("score", 0) < 2]

    executive = (
        f"{case} holds {assets_total} intangible assets identified across the "
        "Four-Leaf Model. The organisation shows emerging licensing readiness, "
        "with strengths in value creation and clear business logic. "
        "Next actions prioritise converting tacit knowledge into explicit artefacts, "
        "evidence capture, and preparing field-of-use licence templates aligned to "
        "IAS 38 and FRAND principles."
    )

    plan_90 = (
        "W1–2: Capture evidence for each asset; map ownership; "
        "prepare IA register.\n"
        "W3–4: Apply 10-Steps discipline — identify, separate, protect, "
        "safeguard, manage, control.\n"
        "W5–6: Prepare FRAND-aligned licence templates (territory, "
        "field-of-use, royalty basis, audit rights).\n"
        "W7–8: Build one licence-ready asset bundle; market test with two pilot clients.\n"
        "W9–12: Close pilots; establish governance log and renewal cycle."
    )

    frand = (
        "1) Grant-back structure — improvements limited to licensed field; "
        "2-3% royalty benchmark; SME subsidy optional.\n"
        "2) Field-of-use split — healthcare royalty tier at 2.2%, "
        "non-commercial tier capped; MFN across peer tier.\n"
        "3) Usage credits tokenised — prepaid blocks, transfer permitted "
        "within group, rate floor and audit log."
    )

    governance = (
        "Nominate IC Lead; monthly IC Board; evidence trails for each asset "
        "(origin, proof, rights, owner). All licences logged and reviewed with "
        "renewal diary and version control. Document risk controls and "
        "audit-ready notes."
    )
